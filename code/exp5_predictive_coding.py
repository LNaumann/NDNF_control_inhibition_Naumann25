...
def exp501_predictive_coding(mean_pop=True, w_hetero=False, reduced=False, pre_inh=False, noise=0.0, save=False, with_NDNF=True, xFF_NDNF=0, rNinit=0, calc_bg_input=False,
                             b=0.12, scale_w_by_p=False, p_scale=None):
    ...
    # w_mean_update.update(dict(NS=0.5, DN=1, PN=0.1, VN=0.5, PS=0.37 ))
    w_mean_update.update(dict(DN=0.8, PN=0))
    ...
    bg_inputs = {'E': 9, 'D': 7.2, 'N': 7.6, 'S': 5.0, 'P': 6.2, 'V': 7.4}


    # initial activities
    rE0, rD0 = 1, 0
    rP0, rS0, rV0 = 4, 4, 4
    # rN0 = 0  (defined elsewhere)
    ...
    model = mb.NetworkModel(N_cells, w_mean.copy(), conn_prob, taus, bg_inputs.copy(), wED=1, flag_w_hetero=w_hetero, flag_pre_inh=pre_inh, flag_with_NDNF=with_NDNF,
                            flag_with_VIP=True, flag_with_PV=True, w_std_rel=0.01)
    if pre_inh:
        model.b = b
    ...
    for i, cond in enumerate(['fp', 'op', 'up']):
        ax2[2, i].plot(t, eval('rE'+cond), c=cPC)
        ax2[3, i].plot(t, eval('rD'+cond), c='k')
        ...
        ax4[2].set(ylabel=r'$\Delta$ inh.', xticks=[0, 1, 2], ylim=[-4, 4.5], yticks=[-3, 0, 3], xticklabels=['P=S', 'P>S', 'P<S'])
    ...
    for i, cond in enumerate(['fp', 'op', 'up']):
        ax[0, i].plot(t, eval('rE'+cond), c=cPC, alpha=0.5, lw=1)
...
exp501_predictive_coding(mean_pop=False, w_hetero=True, reduced=False, pre_inh=True, noise=0.3, save=False, with_NDNF=True, xFF_NDNF=0, rNinit=4, calc_bg_input=True,
                         scale_w_by_p=True)
exp501_predictive_coding(mean_pop=False, w_hetero=True, reduced=False, pre_inh=True, noise=0.3, save=False, with_NDNF=True, xFF_NDNF=1.5, rNinit=6, calc_bg_input=False,
                         scale_w_by_p=True, p_scale=0.52)
